...
int examples::hello_world_ext_12_main(int argc, char** argv)
{
    ...
    window.on_resize.emplace_back([&](int2 size) {
        ...
        uav_heap = gpu.create_descriptor_heap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1,
                                              D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE);
        gpu.device()->CreateUnorderedAccessView(ray_target.get(), nullptr, &uav_descriptor,
    ...
    dx12::Resource quad_vb = gpu.create_buffer(quad_mesh_data.data(), (UINT)quad_mesh_data.size() * sizeof(kl::Vertex),
                                               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    dx12::Resource cube_vb = gpu.create_buffer(cube_mesh_data.data(), (UINT)cube_mesh_data.size() * sizeof(kl::Vertex),
                                               D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    dx12::Resource monke_vb =
        gpu.create_buffer(monke_mesh_data.data(), (UINT)monke_mesh_data.size() * sizeof(kl::Vertex),
                          D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    ...
    dx12::Resource tlas_update_scratch =
        gpu.create_commited_resource(update_scratch_size, D3D12_RESOURCE_STATE_COMMON, D3D12_HEAP_TYPE_DEFAULT,
    ...
    dx12::RootSignature root_signature =
        gpu.create_root_signature({root_parameter0, root_parameter1}, {}, D3D12_ROOT_SIGNATURE_FLAG_NONE);
    ...
    kl::copy<byte>(shader_id_buffer + D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT * 0,
                   pipeline_properties->GetShaderIdentifier(L"rt_g_shader"), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
    kl::copy<byte>(shader_id_buffer + D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT * 1,
                   pipeline_properties->GetShaderIdentifier(L"rt_m_shader"), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
    kl::copy<byte>(shader_id_buffer + D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT * 2,
                   pipeline_properties->GetShaderIdentifier(L"rt_hit_group"), D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
    ...
    while (window.process())
    {
        ...
        gpu.execute([&](kl::GPU12Commands& commands) {
            ...
            commands.dispatch_rays(shader_ids->GetGPUVirtualAddress(), (UINT)target_descriptor.Width,
                                   (UINT)target_descriptor.Height);

            auto back_buffer = gpu.get_back_buffer(gpu.back_buffer_index());
            commands.transition_resource(ray_target, D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            ...
            commands.transition_resource(ray_target, D3D12_RESOURCE_STATE_COPY_SOURCE,
