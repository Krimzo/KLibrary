...
static void draw_background(kl::image& frame)
{
	...
	frame.fill({ 60, 60, 60 });
	frame.draw_rectangle({ 0, lower_limit }, frame.size(), kl::colors::gray, true);
...
int main()
{
	...
	{
		if (const kl::int2 position = window.mouse.position(); position.x >= 0 && position.x < int(window.width()) && position.y > 0 && position.y < get_frame_lower_limit(frame)) {
	...
	{
		object_second_position = kl::float2(window.mouse.position());
	};
