...
static void draw_objects(kl::Image& frame, std::vector<SceneObject>& objects)
{
    for (auto& [radius, position, velocity, color, position_history] : objects)
    {
        ...
        {
            frame.draw_line(
                int2(position_history[i]), int2(position_history[i + 1]),
                kl::RGB(60, 60, 60)
                    .mix(color, kl::unlerp((float)i, (float)start_position_index, (float)position_history.size())));
        }
...
static void process_objects(std::vector<SceneObject>& objects, kl::Timer const& timer, kl::Image const& frame,
                            float2 gravity)
{
...
int examples::trajectories_main(int argc, char** argv)
{
    ...
    window.on_resize.emplace_back([&](int2 size) { frame.resize(size); });
    ...
    while (window.process())
    {
        ...
        if (window.mouse.left.pressed())
        {
            ...
            if (position.x >= 0 && position.x < (int)window.width() && position.y > 0 &&
                position.y < frame_lower_limit(frame))
            {
