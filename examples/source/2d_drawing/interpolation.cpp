...
int examples::interpolation_main(int argc, char** argv)
{
    ...
    while (window.process())
    {
        ...
        for (int x = frame_index - frame.height(), y = 0; y < frame.height(); x++, y++)
        {
            ...
            {
                pixel.r = (byte)kl::Triangle::interpolate(weights[0],
                                                          {(float)colors[0].r, (float)colors[1].r, (float)colors[2].r});
                pixel.g = (byte)kl::Triangle::interpolate(weights[0],
                                                          {(float)colors[0].g, (float)colors[1].g, (float)colors[2].g});
                pixel.b = (byte)kl::Triangle::interpolate(weights[0],
                                                          {(float)colors[0].b, (float)colors[1].b, (float)colors[2].b});
            }
            else if (kl::Triangle::is_in_triangle(weights[1]))
            {
                pixel.r = (byte)kl::Triangle::interpolate(weights[1],
                                                          {(float)colors[0].r, (float)colors[3].r, (float)colors[2].r});
                pixel.g = (byte)kl::Triangle::interpolate(weights[1],
                                                          {(float)colors[0].g, (float)colors[3].g, (float)colors[2].g});
                pixel.b = (byte)kl::Triangle::interpolate(weights[1],
                                                          {(float)colors[0].b, (float)colors[3].b, (float)colors[2].b});
            }
            else
            {
