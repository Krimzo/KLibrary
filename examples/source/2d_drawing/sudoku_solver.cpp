...
struct Index
{
    ...
    constexpr Index(int value) : value(value)
    {
    }

    constexpr Index(int x, int y) : value(x + y * 9)
    {
    }
...
struct Sudoku
{
    ...
    constexpr bool is_solved() const
    {
        ...
        {
            uint16_t block = (*this)[i + 0] | (*this)[i + 1] | (*this)[i + 2] | (*this)[i + 9] | (*this)[i + 10] |
                             (*this)[i + 11] | (*this)[i + 18] | (*this)[i + 19] | (*this)[i + 20];
            if (block != _ALL)
                return false;
        }
        return true;
    ...
    constexpr Possible get_possible(Index index) const
    {
        ...
        for (SudokuPiece p = ONE; p <= NINE; p = SudokuPiece(p << 1))
        {
            ...
            found_same = p & (*this)[sqr_index + 0] || p & (*this)[sqr_index + 1] || p & (*this)[sqr_index + 2] ||
                         p & (*this)[sqr_index + 9] || p & (*this)[sqr_index + 10] || p & (*this)[sqr_index + 11] ||
                         p & (*this)[sqr_index + 18] || p & (*this)[sqr_index + 19] || p & (*this)[sqr_index + 20];
            if (found_same)
                continue;
...
static constexpr std::optional<Sudoku> solve(Sudoku sudoku, Sudoku& displayer, int depth = 0)
{
    while (true)
    {
        ...
        for (int i = 0; i < 81; i++)
        {
            ...
            auto possible = sudoku.get_possible(i);
            if (possible.size() == 0)
                return std::nullopt;
...
VS_DATA v_shader(
	float3 position : KL_Position,
	float3 color : KL_Normal,
	float2 uv : KL_UV
)
{
	VS_DATA data;
...
float4 p_shader(VS_DATA data) : SV_Target
{
	return data.color;
}
)"));
    ...
    void reload(Sudoku const& sudoku)
    {
        ...
        const kl::TextFormat format = gpu.create_text_format(L"JetBrains Mono NL", DWRITE_FONT_WEIGHT_NORMAL,
                                                             DWRITE_FONT_STYLE_NORMAL, font_dips);
