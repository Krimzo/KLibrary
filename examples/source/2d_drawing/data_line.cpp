...
static void draw_offset(kl::Image& frame, std::vector<int2> const& data, float2 lineEquat,
                        kl::RGB color = kl::colors::YELLOW)
{
    if (!data.empty())
    {
        ...
        int2 pos2 =
            int2(data[data_index].x, (int)(data[data_index].x * lineEquat.x + lineEquat.y)) * int2(1, -1) + half_size;
        frame.draw_line(pos1, pos2, color);
    }
}
