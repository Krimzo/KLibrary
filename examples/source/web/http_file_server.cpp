...
struct MyFileServer : kl::HttpApp
{
    ...
    MyFileServer(const std::string root) : root(root)
    {
        ...
        exact_routes["/index.css"] = [](kl::HttpRequest const& request, kl::HttpQuery const& query,
                                        kl::HttpResponse& response) {
  ...
  private:
                            ...
                            kl::HttpResponse& response, fs::path const& fs_path)
    {
        ...
        const std::string file_name_str =
            std::string(fs_path.has_filename() ? kl::format("My File Server: ", fs_path.filename().generic_string())
                                               : "My File Server");
        const std::string file_path_str =
            std::string(fs_path.has_filename() ? fs::relative(fs_path, root).generic_string() : "My File Server");
