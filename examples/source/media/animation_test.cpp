...
struct State
{
    ...
    State(const int2 frame_size)
    {
        ...
        parts.emplace_back(3.0f, [this](float time_perc) {
            ...
            gpu.raster_batch = {
                new kl::RasterText{labels_text_format, "a", FIRST_COLOR,
                                   scale_ndc((triangles[0].b.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "b", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "c", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].b.position.xy()) * 0.5f)},
        ...
        parts.emplace_back(5.0f, [this](float time_perc) {
            ...
            gpu.raster_batch = {
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "a * ", time_perc, "a"), FIRST_COLOR,
                                   scale_ndc((triangles[0].b.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "b * ", time_perc, "a"), FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "c * ", time_perc, "a"), FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].b.position.xy()) * 0.5f)},
        ...
        parts.emplace_back(3.0f, [this](float time_perc) {
            ...
            gpu.raster_batch = {
                new kl::RasterText{labels_text_format, "aa", FIRST_COLOR,
                                   scale_ndc((triangles[0].b.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "ba", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "ca", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].b.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "a", SECOND_COLOR,
                                   scale_ndc((triangles[1].b.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "b", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "c", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].b.position.xy()) * 0.5f)},
        ...
        parts.emplace_back(5.0f, [this](float time_perc) {
            ...
            triangle.c.position.xy() =
                triangle.a.position.xy() +
                ((triangle.c.position.xy() - triangle.a.position.xy()) * kl::lerp(time_perc, 1.0f, b));
            triangle.b.position.xy() =
                triangle.a.position.xy() +
                ((triangle.b.position.xy() - triangle.a.position.xy()) * kl::lerp(time_perc, 1.0f, b));

            triangle.c.position.xy() =
                kl::rotate(triangle.c.position.xy() - triangle.a.position.xy(), -kl::lerp(time_perc, 0.0f, 90.0f)) +
                triangle.a.position.xy();
            triangle.b.position.xy() =
                kl::rotate(triangle.b.position.xy() - triangle.a.position.xy(), -kl::lerp(time_perc, 0.0f, 90.0f)) +
                triangle.a.position.xy();
            ...
            gpu.raster_batch = {
                ...
                new kl::RasterText{labels_text_format, "ca", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].b.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "a * ", time_perc, "b"), SECOND_COLOR,
                                   scale_ndc((triangles[1].b.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "b * ", time_perc, "b"), SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "c * ", time_perc, "b"), SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].b.position.xy()) * 0.5f)},
                new kl::RasterText{
                    info_text_format, "Rotate, move and scale\ntriangle 2 by b.", kl::colors::WHITE, {-0.75f, 0.0f}},
            };
        });
        ...
        parts.emplace_back(3.0f, [this](float time_perc) {
            ...
            gpu.raster_batch = {
                ...
                new kl::RasterText{labels_text_format, "ca", FIRST_COLOR,
                                   scale_ndc((triangles[0].a.position.xy() + triangles[0].b.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "ab", SECOND_COLOR,
                                   scale_ndc((triangles[1].b.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "bb", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "cb", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].b.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "a", THIRD_COLOR,
                                   scale_ndc((triangles[2].b.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "b", THIRD_COLOR,
                                   scale_ndc((triangles[2].a.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "c", THIRD_COLOR,
                                   scale_ndc((triangles[2].a.position.xy() + triangles[2].b.position.xy()) * 0.5f)},
        ...
        parts.emplace_back(5.0f, [this](float time_perc) {
            ...
            const float2 reflect_c =
                triangle.a.position.xy() + kl::reflect(triangle.a.position.xy() - triangle.c.position.xy(),
                                                       triangle.b.position.xy() - triangle.a.position.xy());
        ...
        parts.emplace_back(5.0f, [this](float time_perc) {
            ...
            triangle.c.position.xy() =
                triangle.a.position.xy() +
                ((triangle.c.position.xy() - triangle.a.position.xy()) * kl::lerp(time_perc, 1.0f, c));
            triangle.b.position.xy() =
                triangle.a.position.xy() +
                ((triangle.b.position.xy() - triangle.a.position.xy()) * kl::lerp(time_perc, 1.0f, c));

            triangle.c.position.xy() =
                kl::rotate(triangle.c.position.xy() - triangle.a.position.xy(), kl::lerp(time_perc, 0.0f, 120.0f)) +
                triangle.a.position.xy();
            triangle.b.position.xy() =
                kl::rotate(triangle.b.position.xy() - triangle.a.position.xy(), kl::lerp(time_perc, 0.0f, 120.0f)) +
                triangle.a.position.xy();
            ...
            gpu.raster_batch = {
                ...
                new kl::RasterText{labels_text_format, "cb", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].b.position.xy()) * 0.5f)},
                new kl::RasterText{
                    labels_text_format, kl::wformat(std::fixed, std::setprecision(2), "a * ", time_perc, "c"),
                    THIRD_COLOR,
                    scale_ndc(last_tr_bias + (triangles[2].b.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{
                    labels_text_format, kl::wformat(std::fixed, std::setprecision(2), "b * ", time_perc, "c"),
                    THIRD_COLOR,
                    scale_ndc(last_tr_bias + (triangles[2].a.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format,
                                   kl::wformat(std::fixed, std::setprecision(2), "c * ", time_perc, "c"), THIRD_COLOR,
                                   scale_ndc((triangles[2].a.position.xy() + triangles[2].b.position.xy()) * 0.5f)},
                new kl::RasterText{
        ...
        parts.emplace_back(5.0f, [this](float time_perc) {
            ...
            gpu.raster_batch = {
                ...
                new kl::RasterText{labels_text_format, "cb", SECOND_COLOR,
                                   scale_ndc((triangles[1].a.position.xy() + triangles[1].b.position.xy()) * 0.5f)},
                new kl::RasterText{
                    labels_text_format, "ac", THIRD_COLOR,
                    scale_ndc(last_tr_bias + (triangles[2].b.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{
                    labels_text_format, "bc", THIRD_COLOR,
                    scale_ndc(last_tr_bias + (triangles[2].a.position.xy() + triangles[2].c.position.xy()) * 0.5f)},
                new kl::RasterText{labels_text_format, "cc", THIRD_COLOR,
                                   scale_ndc((triangles[2].a.position.xy() + triangles[2].b.position.xy()) * 0.5f)},
        ...
        labels_text_format =
            gpu.create_text_format(L"JetBrains Mono L", DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, 25.0f);

        info_text_format =
            gpu.create_text_format(L"JetBrains Mono L", DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, 35.0f);
