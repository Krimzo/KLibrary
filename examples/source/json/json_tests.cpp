...
struct Person : js::ObjectSerializable
{
    ...
    Person(std::string const& name, float age) : name(name), age(age)
    {
    }

    void to_object(js::Object& object) override
    {
        object["name"] = js::string(name);
        object["age"] = js::number(age);
    }
...
{
    constexpr auto test = [](js::Container const& container, std::string const& expected) {
    ...
    test(js::Object("{}"), "{}");
    test(js::Object(R"({ " : some \": key\" : ": "some \"string\" in string" })"),
         R"({ " : some \": key\" : ": "some \"string\" in string" })");
    test(js::Object(R"({"data": 16, "person": {"name": "Krimzo", "ages": [22,23]}})"),
         R"({ "data": 16, "person": { "ages": [22, 23], "name": "Krimzo" } })");

    test(js::Array("[]"), "[]");
    test(js::Array(R"([1, 2, 3, [4, 5, {"number":6}, [7, 8, 9 ],], 10])"),
         R"([1, 2, 3, [4, 5, { "number": 6 }, [7, 8, 9]], 10])");
