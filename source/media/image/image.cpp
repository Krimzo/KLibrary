...
kl::Image::Image(int2 size) : m_size(size)
{
...
void kl::Image::draw_triangle(int2 position_a, int2 position_b, int2 position_c, RGB color, bool fill)
{
    ...
    for (int y = position_a.y; y < position_c.y; y++)
    {
        ...
        draw_line({(int)line_x((y < position_b.y) ? flt_pos_a : flt_pos_c, flt_pos_b, (float)y), y},
                  {(int)line_x(flt_pos_a, flt_pos_c, (float)y), y}, color);
    }
}
