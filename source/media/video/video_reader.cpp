...
static uint64_t video_byte_size(ComRef<IMFSourceReader> const& reader)
{
    PROPVARIANT variant{};
...
static int64_t video_duration_100ns(ComRef<IMFSourceReader> const& reader)
{
    PROPVARIANT variant = {};
...
static float video_fps(ComRef<IMFSourceReader> const& reader)
{
    ComRef<IMFMediaType> current_type;
    reader->GetCurrentMediaType(MF_SOURCE_READER_FIRST_VIDEO_STREAM, &current_type);
    if (!current_type)
...
bool kl::VideoReader::read_frame(Image& out, int* out_index, float* out_time) const
{
    ...
    if (FAILED(
            m_reader->ReadSample(MF_SOURCE_READER_FIRST_VIDEO_STREAM, NULL, nullptr, &flags, &time_stamp, &sample)) ||
        !sample)
        return false;
    ...
    RGB const* frame_source = (RGB*)frame_data;
