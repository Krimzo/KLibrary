    ...
    frame_byte_width_(4 * width_), frame_byte_size_(frame_byte_width_ * height_)
{
    ...
    // Ouput media type
    IMFMediaType* media_type_out = nullptr;
    ...
    fail_check_(MFSetAttributeSize(media_type_out, MF_MT_FRAME_SIZE, width_, height_), "Failed to set frame size [output media]");
    fail_check_(MFSetAttributeRatio(media_type_out, MF_MT_FRAME_RATE, fps_, 1), "Failed to set FPS [output media]");
    fail_check_(MFSetAttributeRatio(media_type_out, MF_MT_PIXEL_ASPECT_RATIO, 1, 1), "Failed to set pixel aspect ratio [output media]");
    fail_check_(sink_writer_->AddStream(media_type_out, &stream_index_), "Failed to add output media type stream");

    // Input media type
    IMFMediaType* media_type_in = nullptr;
    ...
    fail_check_(MFSetAttributeSize(media_type_in, MF_MT_FRAME_SIZE, width_, height_), "Failed to set frame size [input media]");
    fail_check_(MFSetAttributeRatio(media_type_in, MF_MT_FRAME_RATE, fps_, 1), "Failed to set FPS [input media]");
    fail_check_(MFSetAttributeRatio(media_type_in, MF_MT_PIXEL_ASPECT_RATIO, 1, 1), "Failed to set pixel aspect ratio [input media]");
    fail_check_(sink_writer_->SetInputMediaType(stream_index_, media_type_in, NULL), "Failed to add input media type stream");
    ...
    safe_release_(media_type_out);
    safe_release_(media_type_in);
}

kl::video_writer::~video_writer()
{
    safe_release_(media_sample_);
    safe_release_(media_buffer_);
    safe_release_(sink_writer_);
}
...
bool kl::video_writer::add_frame(const image& frame)
{
    ...
    media_buffer_->Lock((BYTE**) &out_buffer, nullptr, nullptr);
    ...
    if (!succeeded_(sink_writer_->WriteSample(stream_index_, media_sample_))) {
        return false;
    }
