...
kl::GPU::GPU(HWND window, bool debug, bool has_unordered_access, bool video_support)
{
    ...
    D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, creation_flags, feature_levels,
                      (UINT)std::size(feature_levels), D3D11_SDK_VERSION, &temp_device, nullptr, &temp_context) >>
        verify_result;
    ...
    chain_descriptor.BufferUsage =
        DXGI_USAGE_RENDER_TARGET_OUTPUT | (has_unordered_access ? DXGI_USAGE_UNORDERED_ACCESS : NULL);
    ...
    factory->CreateSwapChainForHwnd(m_device.get(), window, &chain_descriptor, nullptr, nullptr, &temp_chain) >>
        verify_result;
...
void kl::GPU::resize_internal(int2 size, DXGI_FORMAT depth_format)
{
    ...
    m_chain->ResizeBuffers(0, size.x, size.y, DXGI_FORMAT_UNKNOWN, DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING) >> verify_result;
    ...
    for (int i = 0; i < GPU_BUFFER_COUNT; i++)
    {
        ...
        m_d2d1_factory->CreateDxgiSurfaceRenderTarget(surface.get(), target_properties, &m_d2d1_targets[back_index]) >>
            verify_result;
