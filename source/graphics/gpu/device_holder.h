...
struct DeviceHolder : NoCopy
{
    ...
    dx::Buffer create_buffer(dx::BufferDescriptor const* descriptor,
    ...
    dx::Buffer create_structured_buffer(void const* data, UINT element_count, UINT element_size,
                                        bool has_unordered_access = false, bool cpu_read = false) const;
    ...
    dx::Texture create_texture(dx::TextureDescriptor const* descriptor,
                               dx::SubresourceDescriptor const* subresource_data) const;
    dx::Texture create_texture(Image const& image, bool has_unordered_access = false, bool is_target = false) const;
    dx::Texture create_cube_texture(Image const& right, Image const& left, Image const& top, Image const& bottom,
                                    Image const& front, Image const& back) const;
    ...
    dx::InputLayout create_input_layout(CompiledShader const& compiled_shader,
                                        std::initializer_list<dx::LayoutDescriptor> const& descriptors = {}) const;
