...
struct ContextHolder : NoCopy
{
    ...
    void copy_resource_region(dx::Resource const& destination, dx::Resource const& source, int2 src_min, int2 src_max,
                              int2 dst = {0, 0}) const;

    void map_read_resource(dx::Resource const& resource, std::function<void(byte*, uint32_t)> const& func) const;
    void map_write_resource(dx::Resource const& resource, std::function<void(byte*, uint32_t)> const& func,
                            bool discard = true) const;

    void read_from_buffer(void* cpu_buffer, dx::Buffer const& gpu_buffer, SIZE_T byte_size) const;
    void write_to_buffer(dx::Buffer const& gpu_buffer, void const* cpu_buffer, SIZE_T byte_size,
                         bool discard = true) const;

    void read_from_texture(void* cpu_buffer, dx::Texture const& gpu_buffer, int2 cpu_size, UINT element_size) const;
    void write_to_texture(dx::Texture const& gpu_buffer, void const* cpu_buffer, int2 cpu_size, UINT element_size,
                          bool discard = true) const;
    ...
    template <typename T> void set_cb_data(dx::Buffer const& buffer, T const& object) const
    {
    ...
    void draw(dx::Buffer const& vertex_buffer, D3D_PRIMITIVE_TOPOLOGY draw_type = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
              UINT stride = sizeof(Vertex)) const;

    void draw_indexed(UINT index_count, UINT start_index, INT base_vertex) const;
    void draw_indexed(dx::Buffer const& vertex_buffer, dx::Buffer const& index_buffer,
    ...
    void bind_target_depth_views(std::initializer_list<ID3D11RenderTargetView*> const& target_views,
    ...
    void bind_access_view_for_compute_shader(dx::AccessView const& view, UINT slot,
                                             UINT const* initial_counts = nullptr) const;
    ...
    void execute_compute_shader(dx::ComputeShader const& shader, UINT groups_x, UINT groups_y = 1,
                                UINT groups_z = 1) const;
