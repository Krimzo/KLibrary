...
void kl::RasterText::draw(ComRef<ID2D1RenderTarget> const& target, ComRef<ID2D1SolidColorBrush> const& brush) const
{
    ...
    target->DrawTextW(data.data(), (UINT)data.size(), format.get(), layout_rect, brush.get());
}

kl::RasterLine::RasterLine(float2 start, float2 end, float4 color, float stroke_width)
    : start(start), end(end), color(color), stroke_width(stroke_width)
{
}
...
void kl::RasterLine::draw(ComRef<ID2D1RenderTarget> const& target, ComRef<ID2D1SolidColorBrush> const& brush) const
{
    ...
    target->DrawLine((D2D1_POINT_2F const&)from_ndc(start, target_size),
...
kl::Raster::Raster()
{
    ...
    options.debugLevel =
        D2D1_DEBUG_LEVEL(kl::IS_DEBUG ? (D2D1_DEBUG_LEVEL_WARNING | D2D1_DEBUG_LEVEL_ERROR) : D2D1_DEBUG_LEVEL_NONE);
    D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, __uuidof(ID2D1Factory), &options, (void**)&m_d2d1_factory) >>
        verify_result;
    DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory), (IUnknown**)&m_write_factory) >>
        verify_result;
}
                                              ...
                                              std::wstring_view const& locale) const
{
    if (font_size <= 0.0f)
        return {};
    TextFormat format;
    m_write_factory->CreateTextFormat(font_family.data(), nullptr, font_weight, font_style, DWRITE_FONT_STRETCH_NORMAL,
