...
kl::GPU12::GPU12(HWND window, bool debug)
{
    if (debug)
    {
...
kl::dx12::SwapChain kl::GPU12::create_swap_chain(HWND window, dx12::CommandQueue const& command_queue) const
{
    ...
    dx12::SwapChain swap_chain;
    {
        ...
        m_dxgi_factory->CreateSwapChainForHwnd(command_queue.get(), window, &descriptor, nullptr, nullptr,
                                               &temp_chain) >>
            verify_result;
...
kl::dx12::DescriptorHeap kl::GPU12::create_descriptor_heap(D3D12_DESCRIPTOR_HEAP_DESC const* descriptor) const
{
...
kl::dx12::DescriptorHeap kl::GPU12::create_descriptor_heap(D3D12_DESCRIPTOR_HEAP_TYPE type, UINT count,
...
UINT kl::GPU12::get_descriptor_size(D3D12_DESCRIPTOR_HEAP_TYPE type) const
{
...
kl::dx12::CommandList kl::GPU12::create_command_list(dx12::CommandAllocator const& command_allocator) const
{
...
void kl::GPU12::execute(std::function<void(GPU12Commands&)> const& func)
{
    m_commands.reset();
    func(m_commands);
    m_commands.close();
...
kl::dx12::Resource kl::GPU12::get_back_buffer(UINT index) const
{
    return m_back_buffers[index];
}

kl::dx12::DescriptorHandle kl::GPU12::get_render_target(UINT index) const
{
    return {.ptr = m_rtv_descriptor_heap->GetCPUDescriptorHandleForHeapStart().ptr +
                   (SIZE_T)m_rtv_descriptor_size * index};
}
...
kl::dx12::Resource kl::GPU12::create_commited_resource(D3D12_RESOURCE_DESC const* descriptor,
                                                       D3D12_RESOURCE_STATES resource_state,
                                                       D3D12_HEAP_TYPE heap_type) const
{
    const D3D12_HEAP_PROPERTIES heap_props{.Type = heap_type};
    dx12::Resource resource{};
    m_device->CreateCommittedResource(&heap_props, D3D12_HEAP_FLAG_NONE, descriptor, resource_state, nullptr,
                                      IID_PPV_ARGS(&resource)) >>
        verify_result;
    return resource;
}

kl::dx12::Resource kl::GPU12::create_commited_resource(UINT64 byte_size, D3D12_RESOURCE_STATES resource_state,
                                                       D3D12_HEAP_TYPE heap_type, D3D12_RESOURCE_FLAGS flags) const
{
...
kl::dx12::Resource kl::GPU12::create_upload_buffer(void const* data, UINT byte_size) const
{
    dx12::Resource buffer =
        create_commited_resource(byte_size, D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_HEAP_TYPE_UPLOAD);
...
void kl::GPU12::copy(dx12::Resource const& destination, void const* source, UINT byte_size) const
{
    void* buffer = nullptr;
...
void kl::GPU12::copy(void* destination, dx12::Resource const& source, UINT byte_size) const
{
    void* buffer = nullptr;
...
kl::dx12::RootSignature kl::GPU12::create_root_signature(
    ...
    std::initializer_list<D3D12_STATIC_SAMPLER_DESC> const& samplers, D3D12_ROOT_SIGNATURE_FLAGS flags) const
{
    ...
    D3D12SerializeRootSignature(&descriptor, D3D_ROOT_SIGNATURE_VERSION_1, &signature_blob, &error_blob) >>
        verify_result;
    dx12::RootSignature root_signature{};
    m_device->CreateRootSignature(NULL, signature_blob->GetBufferPointer(), signature_blob->GetBufferSize(),
                                  IID_PPV_ARGS(&root_signature)) >>
        verify_result;
    return root_signature;
}

kl::dx12::AccelerationStructure kl::GPU12::create_acceleration_structure(dx12::AccelerationInputs const& inputs,
                                                                         UINT64* update_scratch_size)
{
    ...
    const dx12::Resource scratch =
        create_commited_resource(prebuild_info.ScratchDataSizeInBytes, D3D12_RESOURCE_STATE_COMMON,
                                 D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    const dx12::Resource acceleration_structure = create_commited_resource(
        prebuild_info.ResultDataMaxSizeInBytes, D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE,
        D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
...
kl::dx12::AccelerationStructure kl::GPU12::create_blas(D3D12_RAYTRACING_GEOMETRY_DESC const* geometry_descriptor,
                                                       UINT64* update_scratch_size)
{
    ...
    D3D12_PRIMITIVE_TOPOLOGY_TYPE primitive_topology, std::string* out_vs_error, std::string* out_ps_error) const
{
    ...
    for (auto& [name, format] : input_layout_parts)
        input_layout.emplace_back(name.data(), 0, format, 0, D3D11_APPEND_ALIGNED_ELEMENT,
    ...
    struct RasterPipeline
    {
        ...
        dx12::SubobjectPair<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PRIMITIVE_TOPOLOGY, D3D12_PRIMITIVE_TOPOLOGY_TYPE>
            primitive_topology;
        ...
        dx12::SubobjectPair<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RENDER_TARGET_FORMATS, D3D12_RT_FORMAT_ARRAY>
