...
namespace kl {
	template<typename T> struct vector4 {
		...
		// Subtraction
		ALL void sub(const kl::vector4<T>& obj, kl::vector4<T>& out) const {
			for (int i = 0; i < 4; i++) {
				out[i] = data[i] - obj[i];
			}
		}
		...
		// Multiplication
		ALL void mul(const T& val, kl::vector4<T>& out) const {
			for (int i = 0; i < 4; i++) {
				out[i] = data[i] * val;
			}
		}
		...
		// Division
		ALL void div(const T& val, kl::vector4<T>& out) const {
		...
		ALL void div(const kl::vector4<T>& obj, kl::vector4<T>& out) const {
			for (int i = 0; i < 4; i++) {
				out[i] = data[i] / obj[i];
			}
		}
		...
		// Conditional
		ALL bool equ(const kl::vector4<T>& obj) const {
			for (int i = 0; i < 4; i++) {
				if (data[i] != obj[i]) {
					return false;
				}
		...
		// Sign change
		ALL void abs(kl::vector4<T>& out) const {
			for (uint64 i = 0; i < 4; i++) {
				out[i] = std::abs(data[i]);
			}
		}
		ALL kl::vector4<T> abs() const {
			kl::vector4<T> temp;
			abs(temp);
			return temp;
		}
		ALL void neg(kl::vector4<T>& out) const {
			mul(-1.0f, out);
		}
		ALL kl::vector4<T> neg() const {
			kl::vector4<T> temp;
			neg(temp);
			return temp;
		}
		...
		// Normalized
		ALL void norm(kl::vector4<T>& out) const {
			out = (*this) / len();
		}
		ALL kl::vector4<T> norm() const {
			kl::vector4<T> temp;
			norm(temp);
			return temp;
		}
