...
template <typename T> struct Triangle_T
{
    ...
    constexpr Triangle_T(Vertex_T<T> const& a, Vertex_T<T> const& b, Vertex_T<T> const& c) : a(a), b(b), c(c)
    {
    }
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Triangle_T<T> const& triangle)
{
