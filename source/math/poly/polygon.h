...
namespace kl
{
template <typename T> struct Polygon_T
{
    ...
    template <typename A = T> constexpr A area() const
    {
        A area = {};
    ...
    template <typename A = T> constexpr A spherical_area(A radius) const
    {
        A sum = {};
        const size_t n = coords.size();
        for (size_t i = 0; i < n; i++)
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Polygon_T<T> const& polygon)
{
