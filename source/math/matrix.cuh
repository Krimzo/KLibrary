...
namespace kl {
	template<typename T, uint64 W, uint64 H> struct matrix : public kl::buffer<T, W* H> {
		...
		// Subtraction
		ALL void sub(const kl::matrix<T, W, H>& obj, kl::matrix<T, W, H>& out) const {
			for (uint64 i = 0; i < (W * H); i++) {
				out[i] = (*this)[i] - obj[i];
			}
		}
		...
		// Multiplication
		ALL void mul(const T& val, kl::matrix<T, W, H>& out) const {
			for (uint64 i = 0; i < (W * H); i++) {
				out[i] = (*this)[i] * val;
			}
		}
		...
		template<uint64 S> ALL void mul(const kl::matrix<T, S, W>& obj, kl::matrix<T, S, H>& out) const {
		...
		ALL void mul(const kl::vector3<T>& obj, kl::vector3<T>& out) const {
		...
		ALL void mul(const kl::vector4<T>& obj, kl::vector4<T>& out) const {
		...
		// Comparison
		ALL bool equ(const kl::matrix<T, W, H>& obj) const {
			for (uint64 i = 0; i < (W * H); i++) {
				if ((*this)[i] != obj[i]) {
					return false;
				}
		...
		// Sign change
		ALL void abs(kl::matrix<T, W, H>& out) const {
			for (uint64 i = 0; i < (W * H); i++) {
				out[i] = std::abs((*this)[i]);
			}
		}
		ALL kl::matrix<T, W, H> abs() const {
			kl::matrix<T, W, H> temp;
			abs(temp);
			return temp;
		}
		ALL void neg(kl::matrix<T, W, H>& out) const {
			mul(-1.0f, out);
		}
		ALL kl::matrix<T, W, H> neg() const {
			kl::matrix<T, W, H> temp;
			neg(temp);
			return temp;
		}
		...
		// Transpose
		ALL void tra(kl::matrix<T, H, W>& out) const {
			for (uint64 y = 0; y < H; y++) {
		...
		// Cofactor
		ALL bool cof(uint64 ind, kl::matrix<T, W - 1, H - 1>& out) const {
			if constexpr (W == H) {
				if (ind < (W * H)) {
					uint64 counter = 0;
		...
		ALL kl::matrix<T, W - 1, H - 1> cof(uint64 ind) const {
			kl::matrix<T, W - 1, H - 1> temp;
			cof(ind, temp);
			return temp;
		}
		ALL bool cof(kl::matrix<T, W, H>& out) const {
			if constexpr (W == H) {
				for (uint64 y = 0; y < H; y++) {
					for (uint64 x = 0; x < W; x++) {
		...
		// Adjoint
		ALL bool adj(kl::matrix<T, W, H>& out) const {
			if constexpr (W == H) {
				out = cof().tra();
				return true;
			}
		...
		// Inverse
		ALL bool inv(kl::matrix<T, W, H>& out) const {
			if constexpr (W == H) {
