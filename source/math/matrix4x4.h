...
namespace kl {
	template<typename T> struct matrix4x4 : public kl::matrix<T, 4, 4> {
		...
		ALL matrix4x4() {}
		ALL matrix4x4(const kl::vector4<T>& r0, const kl::vector4<T>& r1, const kl::vector4<T>& r2, const kl::vector4<T>& r3) {
		...
		ALL matrix4x4(const kl::matrix<T, 4, 4>& obj) : kl::matrix<T, 4, 4>(obj) {}

		// Translation matrix
		ALL static kl::matrix4x4<T> translation(const kl::vector3<T>& val) {
			kl::matrix4x4<T> temp;
			temp[3] = T(val.x);
			temp[7] = T(val.y);
			temp[11] = T(val.z);
		...
		// Rotation matrix
		ALL static kl::matrix4x4<T> rotation(const kl::vector3<T>& val) {
		...
		// Scaling matrix
		ALL static kl::matrix4x4<T> scaling(const kl::vector3<T>& size) {
			kl::matrix4x4<T> temp;
			temp[0] = T(size.x);
			temp[5] = T(size.y);
			temp[10] = T(size.z);
		...
		ALL static kl::matrix4x4<T> persp(T fov, T ar, T zNear, T zFar) {
		...
		ALL static kl::matrix4x4<T> ortho(T left, T right, T bottom, T top, T nearZ, T farZ) {
			kl::matrix4x4<T> temp;
		...
		// "Look at" matrix
		ALL static kl::matrix4x4<T> lookAt(const kl::vector3<T>& position, const kl::vector3<T>& target, const kl::vector3<T>& up) {
