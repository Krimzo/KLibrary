...
template <typename T> struct Ray_T
{
    ...
    constexpr Ray_T(Vector3<T> const& origin, Vector3<T> const& direction) : origin(origin)
    {
        set_direction(direction);
    }

    constexpr Ray_T(Vector3<T> const& cam_pos, Matrix4x4<T> const& inv_cam, Vector2<T> const& ndc) : origin(cam_pos)
    {
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Ray_T<T> const& ray)
{
