...
template <typename T> struct Plane_T
{
    ...
    constexpr Plane_T(Vector3<T> const& position, Vector3<T> const& normal) : position(position)
    {
        set_normal(normal);
    }
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Plane_T<T> const& plane)
{
