...
template <typename T> struct Matrix3x3
{
    ...
    template <typename O> constexpr operator Matrix3x3<O>() const
    {
        Matrix3x3<O> result;
        for (int i = 0; i < 9; i++)
            result[i] = O(data[i]);
    ...
    {
        T result = (*this)(0, 0) * ((*this)(1, 1) * (*this)(2, 2) - (*this)(2, 1) * (*this)(1, 2)) -
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Matrix3x3<T> const& mat)
{
