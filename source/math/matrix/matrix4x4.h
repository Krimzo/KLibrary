...
template <typename T> struct Matrix4x4
{
    ...
    template <typename O> constexpr operator Matrix4x4<O>() const
    {
        Matrix4x4<O> result;
        for (int i = 0; i < 16; i++)
            result[i] = O(data[i]);
    ...
    constexpr T determinant() const
    {
        ...
        T result = (*this)(0, 0) * ((*this)(1, 1) * A2323 - (*this)(2, 1) * A1323 + (*this)(3, 1) * A1223) -
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Matrix4x4<T> const& mat)
{
