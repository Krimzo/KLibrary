...
namespace kl {
	template<typename T> struct vector3 {
		...
		// Subtraction
		ALL void sub(const kl::vector3<T>& obj, kl::vector3<T>& out) const {
			for (int i = 0; i < 3; i++) {
				out[i] = data[i] - obj[i];
			}
		}
		...
		// Multiplication
		ALL void mul(const T& val, kl::vector3<T>& out) const {
			for (int i = 0; i < 3; i++) {
				out[i] = data[i] * val;
			}
		}
		...
		// Division
		ALL void div(const T& val, kl::vector3<T>& out) const {
		...
		ALL void div(const kl::vector3<T>& obj, kl::vector3<T>& out) const {
			for (int i = 0; i < 3; i++) {
				out[i] = data[i] / obj[i];
			}
		}
		...
		// Conditional
		ALL bool equ(const kl::vector3<T>& obj) const {
			for (int i = 0; i < 3; i++) {
				if (data[i] != obj[i]) {
					return false;
				}
		...
		// Sign change
		ALL void abs(kl::vector3<T>& out) const {
			for (uint64 i = 0; i < 3; i++) {
				out[i] = std::abs(data[i]);
			}
		}
		ALL kl::vector3<T> abs() const {
			kl::vector3<T> temp;
			abs(temp);
			return temp;
		}
		ALL void neg(kl::vector3<T>& out) const {
			mul(-1.0f, out);
		}
		ALL kl::vector3<T> neg() const {
			kl::vector3<T> temp;
			neg(temp);
			return temp;
		}
		...
		// Normalized
		ALL void norm(kl::vector3<T>& out) const {
			out = (*this) / len();
		}
		ALL kl::vector3<T> norm() const {
			kl::vector3<T> temp;
			norm(temp);
			return temp;
		}
