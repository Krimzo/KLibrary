...
namespace kl {
	template<typename T> struct matrix3x3 : public kl::matrix<T, 3, 3> {
		...
		ALL matrix3x3() {}
		ALL matrix3x3(const kl::vector3<T>& r0, const kl::vector3<T>& r1, const kl::vector3<T>& r2) {
			(*this)[0] = r0.x; (*this)[1] = r0.y; (*this)[2] = r0.z;
			(*this)[3] = r1.x; (*this)[4] = r1.y; (*this)[5] = r1.z;
			(*this)[6] = r2.x; (*this)[7] = r2.y; (*this)[8] = r2.z;
		}
		ALL matrix3x3(const kl::matrix<T, 3, 3>& obj) : kl::matrix<T, 3, 3>(obj) {}

		// Translation matrix
		ALL static kl::matrix3x3<T> translation(const kl::vector2<T>& val) {
			kl::matrix3x3<T> temp;
			temp[2] = val.x;
			temp[5] = val.y;
			return temp;
		...
		// Rotation matrix
		ALL static kl::matrix3x3<T> rotation(const T& val) {
		...
		// Scaling matrix
		ALL static kl::matrix3x3<T> scaling(const kl::vector2<T>& val) {
			kl::matrix3x3<T> temp;
			temp[0] = val.x;
			temp[4] = val.y;
			return temp;
