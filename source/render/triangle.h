...
namespace kl {
	struct triangle {
		...
		ALL kl::float3 weights(const kl::float4& consts, const kl::float2& pos) const {
		...
		ALL kl::float3 weights(const kl::float3& point) const {
		...
		ALL bool in(const kl::float3& weights) const {
			return !(weights.x < 0.0f || weights.y < 0.0f || weights.z < 0.0f);
		}

		ALL float interpolate(const kl::float3& weights, const kl::float3& values) const {
			return weights.dot(values);
		}
		ALL kl::vertex interpolate(const kl::float3& weights) const {
			return {
				kl::float3 {
	...
	// std::cout
	inline std::ostream& operator<<(std::ostream& os, const kl::triangle& obj) {
