...
kl::DirectionalLight::DirectionalLight(GPU const& gpu, int resolution) : m_gpu(gpu)
{
...
kl::float4x4 kl::DirectionalLight::matrix(float4x4 const& inv_cam_mat) const
{
    ...
    const float4x4 light_projection_matrix =
        float4x4::orthographic(max_xyz.x * 2.0f, max_xyz.y * 2.0f, min_z, max_xyz.z);
...
kl::float4x4 kl::DirectionalLight::matrix_cascade(Camera camera, UINT cascade_index) const
{
    ...
    camera.near_plane =
        lerp(cascade_index > 0 ? cascade_ends[cascade_index - 1] : 0.0f, camera_planes.x, camera_planes.y);
