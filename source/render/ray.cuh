...
namespace kl {
	struct ray {
		...
		ALL ray(const kl::camera& cam, const kl::float2& ndc) : ray(cam.position, cam.matrix().inv(), ndc) {}
		...
		ALL bool intersect(const kl::triangle& triangle, kl::float3* outInters = nullptr) const {
		...
		ALL bool intersect(const kl::sphere& sphere) const {
			...
			const float sphRayDis = (sphere.center - rayPoint).len();
