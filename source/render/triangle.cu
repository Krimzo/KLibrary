...
ALL kl::triangle::triangle() {}
ALL kl::triangle::triangle(const kl::vertex& a, const kl::vertex& b, const kl::vertex& c) : a(a), b(b), c(c) {}

ALL kl::float4 kl::triangle::constants() const {
...
ALL kl::float3 kl::triangle::weights(const kl::float4& consts, const kl::float2& pos) const {
...
ALL bool kl::triangle::in(const kl::float4& consts, const kl::float2& pos) const {
	const kl::float3 w = weights(consts, pos);
	return !(w.x < 0.0f || w.y < 0.0f || w.z < 0.0f);
}

ALL float kl::triangle::interpolate(const kl::float4& consts, const kl::float3& values, const kl::float2& pos) const {
	const kl::float3 w = weights(consts, pos);
	return (values.x * w.x + values.y * w.y + values.z * w.z);
}
ALL kl::vertex kl::triangle::interpolate(const kl::float4& consts, const kl::float2& pos) const {
	const kl::float3 w = weights(consts, pos);
	return {
		kl::float3 {
...
ALL kl::vertex kl::triangle::interpolate(const kl::float3& point) const {
