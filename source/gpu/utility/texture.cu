...
kl::dx::texture kl::gpu::newTexture(const kl::image& front, const kl::image& back, const kl::image& left, const kl::image& right, const kl::image& top, const kl::image& bottom) {
	...
	kl::dx::desc::subres texData[6] = {
		{  right.data(), uint(front.width() * sizeof(kl::color)), 0 },
		{   left.data(), uint(front.width() * sizeof(kl::color)), 0 },
		{    top.data(), uint(front.width() * sizeof(kl::color)), 0 },
		{ bottom.data(), uint(front.width() * sizeof(kl::color)), 0 },
		{  front.data(), uint(front.width() * sizeof(kl::color)), 0 },
		{   back.data(), uint(front.width() * sizeof(kl::color)), 0 }
	};
