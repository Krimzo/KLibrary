...
const HANDLE kl::ConsoleInit::_init = []() -> HANDLE {
...
void kl::console::clear()
{
    ...
    GetConsoleScreenBufferInfo(ConsoleInit::_init, &console_screen_info);

    DWORD chars_written{};
    FillConsoleOutputCharacterA(ConsoleInit::_init, ' ', console_screen_info.dwSize.X * console_screen_info.dwSize.Y,
                                {}, &chars_written);

    FillConsoleOutputAttribute(ConsoleInit::_init, FOREGROUND_GREEN | FOREGROUND_RED | FOREGROUND_BLUE,
...
void kl::console::set_cursor_enabled(bool state)
{
    ...
    GetConsoleCursorInfo(ConsoleInit::_init, &cursor_info);
...
void kl::console::set_font(int2 size, std::wstring_view font_name)
{
    ...
    SetCurrentConsoleFontEx(ConsoleInit::_init, false, &cfi);
}
