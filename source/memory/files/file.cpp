...
std::optional<std::string> kl::choose_file(bool save, std::string_view defaul,
                                           std::vector<std::pair<std::string_view, std::string_view>> const& filters,
                                           int* out_index)
{
...
std::optional<std::wstring> kl::wchoose_file(
    bool save, std::wstring_view defaul, std::vector<std::pair<std::wstring_view, std::wstring_view>> const& filters,
    int* out_index)
{
    ...
    if (SUCCEEDED(file_open_dialog->GetFolder(&current_folder)))
    {
        ...
        if (SUCCEEDED(current_folder->GetDisplayName(SIGDN_FILESYSPATH, &current_folder_str)))
        {
            ...
            if (current_folder_path.has_parent_path())
            {
                ...
                if (SUCCEEDED(SHCreateItemFromParsingName(current_folder_path.parent_path().c_str(), nullptr,
                                                          IID_PPV_ARGS(&parent_folder_item))))
    ...
    for (std::string line; std::getline(file, line);)
    {
        ...
        if (parts.size() == 4 && parts.front() == "f")
        {
            for (int i = 1; i <= 3; i++)
            {
                ...
                Vertex vertex;
                if (uint64_t index = parse_int(line_part_parts[0]).value_or(0) - 1;
                    index >= 0 && index < position_data.size())
                    vertex.position = position_data[index];
                if (uint64_t index = parse_int(line_part_parts[2]).value_or(0) - 1;
                    index >= 0 && index < normal_data.size())
                    vertex.normal = normal_data[index];
                if (uint64_t index = parse_int(line_part_parts[1]).value_or(0) - 1;
                    index >= 0 && index < uv_data.size())
