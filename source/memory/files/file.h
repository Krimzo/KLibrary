...
struct File : NoCopy
{
    ...
    template <typename T> T read(uint64_t* opt_out) const
    {
        T result{};
        if (opt_out)
    ...
    template <typename T> uint64_t read(T& object) const
    {
        return read<T>(&object, 1);
    }

    template <typename T> uint64_t read(T* buffer, uint64_t count) const
    {
        if (!m_file)
            return 0;
    ...
    template <typename T> uint64_t write(T const& object)
    {
        return write<T>(&object, 1);
    }

    template <typename T> uint64_t write(T const* buffer, uint64_t count)
    {
        if (!m_file)
            return 0;
...
std::optional<std::string> choose_file(
    ...
    std::vector<std::pair<std::string_view, std::string_view>> const& filters = {{"All Files", ".*"}},
    int* out_index = nullptr);
std::optional<std::wstring> wchoose_file(
    bool save, std::wstring_view defaul = {},
