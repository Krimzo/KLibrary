...
namespace kl
{
template <typename T> constexpr void const* _addr(T const& obj)
{
...
template <typename T> struct ComRef
{
    ...
    explicit ComRef(T* instance) : m_instance(instance)
    {
    }
    ...
    ComRef(ComRef const& other) : m_instance(other.m_instance)
    {
        increase_count();
    }
    ...
    ComRef(ComRef&& other) noexcept : m_instance(other.m_instance)
    {
        other.clear();
    }
