...
struct TCPSocket : NoCopy
{
    ...
    template <typename T> bool send(T const& obj) const
    {
        return send(&obj, sizeof(T)) == sizeof(T);
    }

    template <typename T> bool receive(T& out_obj) const
    {
        return receive(&out_obj, sizeof(T)) == sizeof(T);
    }

    template <typename T> std::optional<T> receive() const
    {
        T obj{};
        if (receive<T>(obj))
            return obj;
...
struct UDPSocket : NoCopy
{
    ...
    template <typename T> bool send(T const& obj, Address const& address) const
    {
        return send(&obj, sizeof(T), address) == sizeof(T);
    }

    template <typename T> bool receive(T& out_obj, Address& out_address) const
    {
        return receive(&out_obj, sizeof(T), out_address) == sizeof(T);
    }

    template <typename T> std::optional<T> receive(Address& out_address) const
    {
        T obj{};
