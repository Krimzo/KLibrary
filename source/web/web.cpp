...
std::optional<std::string> kl::download_website(std::string_view const& url, int buffer_size)
{
    ...
    Internet address = InternetOpenUrlA(connection, url.data(), nullptr, 0,
    ...
    for (DWORD bytes_read = 0;
         InternetReadFile(address, buffer.data(), (DWORD)buffer.size(), &bytes_read) && bytes_read > 0;)
                          ...
                          DWORD optional_size)
{
    ...
    Internet request =
        HttpOpenRequestA(internet, verb.data(), object.data(), nullptr, nullptr, nullptr, INTERNET_FLAG_SECURE, 0);
    if (!request)
        return 3;
