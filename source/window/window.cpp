...
kl::Window::Window(std::string_view name) : m_name(name)
{
    ...
    m_window = CreateWindowExA(NULL, name.data(), name.data(), WS_OVERLAPPEDWINDOW, new_position.x, new_position.y,
                               new_size.x, new_size.y, nullptr, nullptr, m_instance, nullptr);
...
void kl::Window::set_position(int2 position) const
{
    if (fullscreened())
        return;
    SetWindowPos(m_window, nullptr, position.x, position.y, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
}
...
void kl::Window::draw_pixel_data(RGB const* data, int2 size, int2 position) const
{
    ...
    StretchDIBits(m_device_context, position.x, position.y, size.x, size.y, 0, 0, size.x, size.y, data, &bitmap_info,
...
void kl::Window::set_dark_mode(bool enabled) const
{
    ...
    DwmSetWindowAttribute(m_window, DWMWINDOWATTRIBUTE::DWMWA_USE_IMMERSIVE_DARK_MODE, &mode, sizeof(BOOL));
}
