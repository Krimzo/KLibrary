...
namespace kl::dx12
{
template <D3D12_PIPELINE_STATE_SUBOBJECT_TYPE Type, typename Value> struct alignas(void*) SubobjectPair
{
...
struct GPU12 : NoCopy, ShaderCompiler
{
    ...
    dx12::DescriptorHeap create_descriptor_heap(
    ...
    dx12::Resource create_commited_resource(D3D12_RESOURCE_DESC const* descriptor, D3D12_RESOURCE_STATES resource_state,
                                            D3D12_HEAP_TYPE heap_type = D3D12_HEAP_TYPE_DEFAULT) const;

    dx12::Resource create_commited_resource(UINT64 byte_size, D3D12_RESOURCE_STATES resource_state,
    ...
    dx12::Resource create_buffer(void const* data, UINT byte_size,
                                 D3D12_RESOURCE_STATES final_state = D3D12_RESOURCE_STATE_COMMON);

    template <typename T> dx12::VertexBuffer create_vertex_buffer(dx12::Resource const& resource) const
    {
    ...
    dx12::RootSignature create_root_signature(
        ...
        D3D12_ROOT_SIGNATURE_FLAGS flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT) const;

    template <typename T> dx12::PipelineState create_pipeline_state(T const* pipeline_state_desc) const
    {
    ...
    dx12::AccelerationStructure create_acceleration_structure(dx12::AccelerationInputs const& inputs,
                                                              UINT64* update_scratch_size = nullptr);
    dx12::AccelerationStructure create_blas(D3D12_RAYTRACING_GEOMETRY_DESC const* geometry_descriptor,
                                            UINT64* update_scratch_size = nullptr);
    dx12::AccelerationStructure create_triangle_blas(dx12::Resource const& vertex_buffer,
                                                     UINT vertex_stride = sizeof(kl::Vertex));
    ...
    dx12::StateObject create_default_raytracing_pipeline(std::string_view const& compiled_shaders,
                                                         ...
                                                         UINT max_payload_size = 16) const;
