...
void kl::ContextHolder::copy_resource_region(dx::Resource const& destination, dx::Resource const& source, int2 src_min,
                                             int2 src_max, int2 dst) const
{
...
void kl::ContextHolder::map_read_resource(dx::Resource const& resource,
                                          std::function<void(byte*, uint32_t)> const& func) const
{
...
void kl::ContextHolder::map_write_resource(dx::Resource const& resource,
                                           std::function<void(byte*, uint32_t)> const& func, bool discard) const
{
    dx::MappedSubresourceDescriptor mapped_subresource{};
    m_context->Map(resource.get(), 0, discard ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_WRITE, NULL, &mapped_subresource) >>
        verify_result;
    func((byte*)mapped_subresource.pData, mapped_subresource.RowPitch);
    m_context->Unmap(resource.get(), 0);
}
...
void kl::ContextHolder::write_to_buffer(dx::Buffer const& gpu_buffer, void const* cpu_buffer, SIZE_T byte_size,
                                        bool discard) const
{
    dx::MappedSubresourceDescriptor mapped_subresource{};
    m_context->Map(gpu_buffer.get(), 0, discard ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_WRITE, NULL,
                   &mapped_subresource) >>
        verify_result;
...
void kl::ContextHolder::read_from_texture(void* cpu_buffer, dx::Texture const& gpu_buffer, int2 cpu_size,
                                          UINT element_size) const
{
    dx::MappedSubresourceDescriptor mapped_subresource{};
    m_context->Map(gpu_buffer.get(), 0, D3D11_MAP_READ, NULL, &mapped_subresource) >> verify_result;
...
void kl::ContextHolder::write_to_texture(dx::Texture const& gpu_buffer, void const* cpu_buffer, int2 cpu_size,
                                         UINT element_size, bool discard) const
{
    dx::MappedSubresourceDescriptor mapped_subresource{};
    m_context->Map(gpu_buffer.get(), 0, discard ? D3D11_MAP_WRITE_DISCARD : D3D11_MAP_WRITE, NULL,
                   &mapped_subresource) >>
...
void kl::ContextHolder::draw_indexed(dx::Buffer const& vertex_buffer, dx::Buffer const& index_buffer,
                                     D3D_PRIMITIVE_TOPOLOGY draw_type, UINT stride) const
{
    set_draw_type(draw_type);
    bind_vertex_buffer(vertex_buffer, 0, 0, stride);
...
void kl::ContextHolder::bind_target_depth_views(std::initializer_list<ID3D11RenderTargetView*> const& target_views,
                                                dx::DepthView const& depth_view) const
{
...
void kl::ContextHolder::bind_access_view_for_compute_shader(dx::AccessView const& view, UINT slot,
                                                            UINT const* initial_counts) const
{
...
void kl::ContextHolder::execute_compute_shader(dx::ComputeShader const& shader, UINT groups_x, UINT groups_y,
                                               UINT groups_z) const
{
    m_context->CSSetShader(shader.get(), nullptr, 0);
    m_context->Dispatch(groups_x, groups_y, groups_z);
