...
struct CBuffer
{
    ...
    CBuffer(GPU const* gpu);

    void upload(void const* data, UINT byte_size);
...
namespace kl
{
template <typename S> struct ShaderHolder : private CBuffer
{
    S shader = {};

    ShaderHolder(GPU const* gpu = nullptr) : CBuffer(gpu)
    {
    }
    ...
    template <typename T> void upload(T const& object, int index = 0)
    {
