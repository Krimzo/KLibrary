...
struct RasterText : RasterItem
{
    ...
    RasterText() = default;
    RasterText( TextFormat const& format, std::string_view const& data, float4 const& color, float2 tl, float2 br = { 1.0f, -1.0f }, HAlign h_align = DWRITE_TEXT_ALIGNMENT_LEADING, VAlign v_align = DWRITE_PARAGRAPH_ALIGNMENT_NEAR );
    RasterText( TextFormat const& format, std::wstring_view const& data, float4 const& color, float2 tl, float2 br = { 1.0f, -1.0f }, HAlign h_align = DWRITE_TEXT_ALIGNMENT_LEADING, VAlign v_align = DWRITE_PARAGRAPH_ALIGNMENT_NEAR );
...
struct RasterRectangle : RasterItem
{
    ...
    RasterRectangle( float2 const& top_left, float2 const& bottom_right, float4 const& color, bool fill, float2 const& corner_radius = {}, float stroke_width = 1.0f );
...
struct RasterEllipse : RasterItem
{
    ...
    RasterEllipse( float2 const& center, float2 const& radius, float4 const& color, bool fill, float stroke_width = 1.0f );
