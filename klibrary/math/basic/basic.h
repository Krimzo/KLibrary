...
namespace kl
{
template <typename T = float> consteval T pi() noexcept
{
    return T(3.1415926535897932385);
}

template <typename T = float> consteval T to_radians() noexcept
{
    return T(pi<double>() / 180.0);
}

template <typename T = float> consteval T to_degrees() noexcept
{
    return T(180.0 / pi<double>());
}

template <typename T = float> consteval T to_float_rgb() noexcept
{
    return T(1.0 / 255.0);
}

template <typename T = float> consteval T to_byte_rgb() noexcept
{
    return T(255.0 / 1.0);
}
} // namespace kl
