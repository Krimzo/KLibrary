...
template <typename T> struct Matrix2x2
{
    ...
    template <typename O> constexpr operator Matrix2x2<O>() const
    {
        Matrix2x2<O> result;
        for (int i = 0; i < 4; i++)
            result[i] = O(data[i]);
    ...
    constexpr void operator+=(Matrix2x2<T> const& other)
    {
        for (int i = 0; i < 4; i++)
            data[i] += other[i];
    }
    ...
    constexpr Vector2<T> operator*(Vector2<T> const& vec) const
    {
        Vector2<T> result;
        for (int y = 0; y < 2; y++)
            for (int i = 0; i < 2; i++)
    ...
    {
        T result = data[0] * data[3] - data[1] * data[2];
        return T(1) / result;
    }
};
} // namespace kl
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Matrix2x2<T> const& mat)
{
