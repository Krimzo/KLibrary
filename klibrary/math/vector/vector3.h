...
template <typename T> struct Vector3
{
    ...
    explicit constexpr Vector3(T value) : x(value), y(value), z(value)
    {
    }
    ...
    constexpr Vector3(Vector2<T> const& vec, T z) : x(vec.x), y(vec.y), z(z)
    {
    }

    constexpr Vector3(T x, Vector2<T> const& vec) : x(x), y(vec.x), z(vec.y)
    {
    }
    ...
    template <typename O> constexpr operator Vector3<O>() const
    {
        return {O(x), O(y), O(z)};
    }
    ...
    {
        return x >= lower_incl.x && x < upper_excl.x && y >= lower_incl.y && y < upper_excl.y && z >= lower_incl.z &&
               z < upper_excl.z;
    }
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Vector3<T> const& vec)
{
