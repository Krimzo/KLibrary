...
template <typename T> struct Vector4
{
    ...
    explicit constexpr Vector4(T value) : x(value), y(value), z(value), w(value)
    {
    }
    ...
    constexpr Vector4(Vector2<T> const& vec, T z, T w) : x(vec.x), y(vec.y), z(z), w(w)
    {
    }

    constexpr Vector4(T x, Vector2<T> const& vec, T w) : x(x), y(vec.x), z(vec.y), w(w)
    {
    }

    constexpr Vector4(T x, T y, Vector2<T> const& vec) : x(x), y(y), z(vec.x), w(vec.y)
    {
    }

    constexpr Vector4(Vector2<T> const& vec1, Vector2<T> const& vec2) : x(vec1.x), y(vec1.y), z(vec2.x), w(vec2.y)
    {
    }

    constexpr Vector4(Vector3<T> const& vec, T w) : x(vec.x), y(vec.y), z(vec.z), w(w)
    {
    }

    constexpr Vector4(T x, Vector3<T> const& vec) : x(x), y(vec.x), z(vec.y), w(vec.z)
    {
    }
    ...
    template <typename O> constexpr operator Vector4<O>() const
    {
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Vector4<T> const& vec)
{
