...
template <typename T> struct Vector2
{
    ...
    explicit constexpr Vector2(T value) : x(value), y(value)
    {
    }
    ...
    template <typename O> constexpr operator Vector2<O>() const
    {
        return {O(x), O(y)};
    }
    ...
    {
        return x >= lower_incl.x && x < upper_excl.x && y >= lower_incl.y && y < upper_excl.y;
    }
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Vector2<T> const& vec)
{
