...
template <typename T> struct Quaternion_T
{
    ...
    constexpr Quaternion_T(T x, T y, T z) : i(x), j(y), k(z)
    {
    }

    constexpr Quaternion_T(T r, T i, T j, T k) : r(r), i(i), j(j), k(k)
    {
    }

    explicit constexpr Quaternion_T(Vector3<T> const& axis, T angle) : r((T)cos_d(angle * 0.5))
    {
...
namespace kl
{
template <typename T> std::ostream& operator<<(std::ostream& stream, Quaternion_T<T> const& quat)
{
