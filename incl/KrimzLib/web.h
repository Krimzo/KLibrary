...
namespace kl {
	...
	public:
		...
		static kl::bytes GetWebsiteData(std::string url, int bufferSize = 16384) {
			// Create browser
			HINTERNET connection = InternetOpenA("CoolBrowser", INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, NONE);
			if (!connection) {
			...
			// Open url
			HINTERNET openAddress = InternetOpenUrlA(connection, url.c_str(), nullptr, NONE, INTERNET_FLAG_PRAGMA_NOCACHE | INTERNET_FLAG_KEEP_CONNECTION, NONE);
			if (!openAddress) {
