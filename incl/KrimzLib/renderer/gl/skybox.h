...
namespace kl {
	namespace gl {
		...
		private:
			kl::gl::mesh* box_mes = nullptr;
			kl::gl::texture* box_tex = nullptr;
			kl::gl::shaders* box_sha = nullptr;
			...
			void build(kl::image& front, kl::image& back, kl::image& left, kl::image& right, kl::image& top, kl::image& bottom) {
				...
				box_mes = new kl::gl::mesh(boxVertices);

				// Generating the box texture
				box_tex = new kl::gl::texture(front, back, left, right, top, bottom);

				// Compiling skybox shaders
				box_sha = new kl::gl::shaders(vertSource, fragSource);
		...
		std::string kl::gl::skybox::vertSource = R"(
			#version 330
		...
		std::string kl::gl::skybox::fragSource = R"(
			#version 330
		...
		// Skybox box vertices
		std::vector<kl::vertex> kl::gl::skybox::boxVertices = {
