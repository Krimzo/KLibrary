	...
	public:
		...
		public:
			...
			void Create(int port) {
				if (!created) {
					// Create a socket
					serverSocket = socket(AF_INET, SOCK_STREAM, NONE);
			...
			void WaitForClient() {
				if (!clientConnected) {
					...
					if (!getnameinfo((sockaddr*)&client, clientSize, host, NI_MAXHOST, service, NI_MAXSERV, NONE)) {
			...
			void SendData(kl::bytes& data) {
				if (created && clientConnected) {
					if (send(clientSocket, (char*)&data[0], (int)data.size(), NONE) == SOCKET_ERROR) {
			...
			void ReceiveData(kl::bytes& dataBuffer) {
				if (created && clientConnected) {
					...
					if (recv(clientSocket, (char*)&dataBuffer[0], (int)dataBuffer.size(), NONE) == SOCKET_ERROR) {
						clientConnected = false;
					}
				}
			}
		...
		public:
			...
			void Connect(std::string serverIP, int serverPort) {
				...
				// Create a socket
				clientSocket = socket(AF_INET, SOCK_STREAM, NONE);
				if (clientSocket == INVALID_SOCKET) {
